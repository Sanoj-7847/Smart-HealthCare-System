...
public class AppointmentController {
    ...
    public ResponseEntity<ApiResponse<AppointmentResponse>> book(@Valid @RequestBody AppointmentRequest req,
        ...
        return ResponseEntity.ok(ApiResponse.success(appointmentService.bookAppointment(req, uid(ud)), "Appointment booked!"));
    }
    @GetMapping("/my")
    ...
    @GetMapping("/doctor/today") @PreAuthorize("hasRole('DOCTOR')")
    ...
    public ResponseEntity<ApiResponse<AppointmentResponse>> cancel(@PathVariable Long id,
            @AuthenticationPrincipal UserDetails ud) {
        return ResponseEntity.ok(ApiResponse.success(appointmentService.cancelAppointment(id, uid(ud)), "Cancelled"));
    }
    @PutMapping("/{id}/reschedule") @PreAuthorize("hasRole('PATIENT')")
    public ResponseEntity<ApiResponse<AppointmentResponse>> reschedule(@PathVariable Long id,
        ...
        return ResponseEntity.ok(ApiResponse.success(appointmentService.rescheduleAppointment(id, req, uid(ud)), "Rescheduled"));
    }
    @PatchMapping("/{id}/complete") @PreAuthorize("hasRole('DOCTOR')")
